...
## New features
  ...
  1. Add smart treatment of Cntrl+C(`KeyboardInterrupt`-exception) for all parallel work-managers
...
## New features
  ...
  1. Add Adversarial Vaildation tools to compare (weighted) dataset. It can be used for Goodness-of-fit and for reweighting quality 
...
## New features
  ...
  1. Add `Ostap::Math::AsymVars` utility/base class for asymmetry-related parameters, e.g. for asymmetic peaks
...
## New features
  ...
  1. fix `shared_entries` and `shared_data` methods for `RooDataSet`
...
## Backward incompatible
 ...
 1. remove comamd line argumetn fo th elist of configuration files 
...
## New features
   ...
   1. remove the upper limit on the number of used cpus 
   1. re-write `summary_graph`, make it much more robust & more intuitive
   1. `Ostap::StatVar` : extend the error message for `Ostap::Assert`
   1. Ostap::AddVar`  : allow to add RooReal
   1. add `|`into the list of symbols that are not allowed for "good" names
