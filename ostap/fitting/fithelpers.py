...
class VarMaker (object) :
    ...
    def fatal     ( self , message , *args , **kwargs ) :
        """ Produce FATAL   message using the local logger"""
    ...
    def attention ( self , message , *args , **kwargs ) :
        """ Produce ATTENTION  message using the local logger"""
        return self.logger.attention ( message , *args , **kwargs )
    ...
    def vars_to_asymmetry ( self             ,
        ...
        ## It is symmetric
        if isinstance ( var1 , ROOT.RooAbsReal ) and ( var1 is var2 ) :
            return var1 , ZERO 
    ...
    def vars_from_asymmetry ( self         ,
        ...
        if isinstance ( hsumvar , ROOT.RooAbsReal ) and asymvar is ZERO :
            return hsumvar , hsumvar
...
args_constraints   = 'constraint' , 'constraints' , 'pars' , 'params' , 'parameters'
...
class FitHelper(VarMaker) :
    ...
    # =========================================================================
    ## Getter for certain fit components from the provided list 
...
# =============================================================================
## @class AsymVars
#  Helper structure to deal with the cluster of "asymmetry" variables
#  - the first   variable: v1
#  - the second  variable: v2
#  - half sum            : (v1+v2)/2
#  - asymmetry  variaable kappa = ( v1-v2)/(v1+v2)
#  - skew varibale psi :  kappa = tanh(psi)   
class AsymVars(object) :
    """ Helper structure to deal with the cluster of "asymmetry" variables
    - the first   variable: v1
    - the second  variable: v2
    - half sum            : (v1+v2)/2
    - asymmetry  variable kappa = ( v1-v2)/(v1+v2)
    - skew varibale psi :  kappa = tanh(psi)   
    """
    def __init__ ( self      ,
                   var_maker , 
        ...
        assert isinstance ( var_maker , VarMaker ) , "Invalid `var_maker'!"        
        self.__var_maker = var_maker
        ...
        ## 2nd valid trivial  case : single var2 , everything else is None 
        ...
        ## 3rd valid trivial  case : var1 == var2 
            ...
            if not all ( v is None for v in ( halfsum , kappa , psi ) ) :
                self.__var_maker.error ( 'case (3): not all extra variables are None. ignore!' ) 
                
            self.__var1    = var1
            self.__var2    = var2
            self.__halfsum = var1 ## ATTENTION!
            self.__kappa   = ZERO 
            self.__psi     = ZERO 

        ## (4) var1 is specified , the rest is None  
        elif ( not var1 is None ) and all ( v is None for v in ( var2 , halfsum , kappa , psi ) ) :
        ...
        ## (5) var2 is specified , the rest is None  
        elif ( not var2 is None ) and all ( v is None for v in ( var1 , halfsum , kappa , psi ) ) :
        ...
        ## (6) two variables are specified, the rest is None  
            ...
            self.__var1    = var1
            self.__var2    = var2
            self.__halfsum = halfsum 
            self.__kappa   = kappa  
            self.__psi     = psi   

        # (7) half-sum and asymmetry :
            ...
            if kappa is ZERO :
                ...
                psi         = ZERO 
                var1 , var2 = halfsum, halfsum 

            else :
                
                names = halfsum.name , kappa.name , self.name 
                ## get the variables 
                var1 , var2 = self.__var_maker.vars_from_asymmetry ( halfsum , kappa , v1name = v1_name , v2name = v2_name )
        ...
        # (8) half-sum and skew 
            ...
            halfsum = self.make_var ( halfsum  , halfsum_name , 'halfsum of %s & %s variables'  % ( v1_name , v2_name ) , False , halfsum )
            psi     = self.make_var ( psi      , psi_name     , 'skew    %s & %s variables'     % ( v1_name , v2_name ) , False , psi     )
            ...
            if psi is ZERO :
                ...
                kappa      = Ostap.MoreRooFit.Tanh ( kappa_name , "asymmetry: %s & %s " % ( v1_name , v2_name ) , psi , 1.0 )
            ...
            raise TypeError ( 'Invalid set of variables for %s' % var_name )

        ## final adjustnment :
        psi_min  , psi_max   = -16         , +16
        kappa_min, kappa_max = -1 + 1.e-12 , +1 - 1.e-12
        adjust = ( self.psi , psi_min , psi_max ) , ( self.kappa , kappa_min , kappa_max ) 
        for v , mn , mx in adjust :
            
            if hasattr ( v , 'hasMin' ) and hasattr ( v , 'setMin' ) :
                has_min = v.hasMin()
                if   not has_min      : v.setMin ( mn )
                elif v.getMin() < mn  : v.setMin ( mn )
                
            if hasattr ( v , 'hasMax' ) and hasattr ( v , 'setMax' ) : 
                has_max = v.hasMax()
                if   not has_max      : v.setMax ( mx )
                elif v.getMax() > mx  : v.setMax ( mx )
            
    # =========================================================================
    ## create the variable from description
    #  @see VarMaker.make_var 
    def make_var ( self , *args , **kwargs ) :
        """ Create the variable from description
        - see VarMaker.make_var 
        """
        return self.__var_maker.make_var ( *args , **kwargs )

    # =========================================================================
    ## create the variable from description
    #  @see VarMaker.roo_name 
    def new_roo_name ( self , *args , **kwargs ) :
        """ Create the variable from description
        - see VarMaker.roo_name 
        """
        return self.__var_maker.new_roo_name ( *args , **kwargs )

    # =========================================================================
    ## set value fo the variable
    #  @see VarMaker.set_value 
    def set_value ( self , variable , value , *args , **kwargs ) :
        """ Set value fo the variable
        - see VarMaker.set_value 
        """
        return self.__var_maker.set_value ( variable , value , *args , **kwargs )
    ...
    @property 
    def var1 ( self ) :
        """`var2` : the first/left variable v1/vL """
        return self.__var1
    @var1.setter
    def var1 ( self , value ) :
    ...
    @property 
    def var2 ( self ) :
        """`var2` : the second/right variable v2/vR """
        return self.__var2
    @var2.setter
    def var2 ( self , value ) :
...
class TwoSigmas(object) :
    ...
    def __init__ ( self          ,
        ...
        assert isinstance  ( sigmaL , ROOT.RooAbsReal ) , "Invalid type of `sigmaL': %s" % typename ( sigmaL )
        ## asymmetry cluster:        
        if sigmaR is None : self.__AV_SIGMA_2 = self.asymmetry_vars ( 'sigma' , var1 = sigmaL , var2 = sigmaL )
        else              : self.__AV_SIGMA_2 = self.asymmetry_vars ( 'sigma' , var1 = sigmaL , var2 = sigmaR )
...
class SigmaLR(object) :
    ...
    def __init__ ( self         ,
        ...
        assert isinstance  ( sigma , ROOT.RooAbsReal ) , "Invalid type of `sigma': %s" % typename ( sigma )
        ## asymmetry cluster:
        if psi is None : self.__AV_SIGMA_LR = self.asymmetry_vars ( 'sigma' , var1    = sigma , var2 = sigma )
        else           : self.__AV_SIGMA_LR = self.asymmetry_vars ( 'sigma' , halfsum = sigma , psi = psi    )
        
    @property
    def sigmaL ( self ) :
        """'sigmaL' :  (left) sigma """
        return self.__AV_SIGMA_LR.var1
    @property
    def sigmaR ( self ) :
        """'sigmaR' :  (right) sigma """
        return self.__AV_SIGMA_LR.var2
    
    @property
    def kappa  ( self ) :
        """'kappa' : left/right asymmetry for sigmaL kappa = tanh(psi) """
        return self.__AV_SIGMA_LR.kappa 

    @property
    def psi ( self ) :
        """'psi' : left/right asymmetry for sigma: kappa = tanh(psi) """
        return self.__AV_SIGMA_LR.psi         
    @psi.setter
    def psi ( self , value ) :
