...
class APDF1 ( object ) :
    ...
    # =========================================================================
    ## Helper method to create profile likelihood calculator 
    #  @code
    #  dataset       = ...
    #  pdf           = ,,,
    #  plc , mc , ws = pdf._plc_mc_ws ( 'sigma' ,  0.95 , dataset )    
    #  @endcode 
    #  @see RooStats::ProfileLikelihoodCalculator
    def _plc_mc_ws ( self , param , dataset ) :
        """ Helper method to create profile likelihood calculator 
        >>> dataset       = ...
        >>> pdf           = ,,,
        >>> plc , mc , ws = pdf._plc_mc_ws ( 'sigma' ,  0.95 , dataset )    
        - see `ROOT.RooStats.ProfileLikelihoodCalcualtor`
        """
        assert param in self , 'Invalid parameter %s'        % param

        pp = self.parameter    ( param ) 
        ## create workspace
        ws = ROOT.RooWorkspace         ( 'WS' , 'Workspace for PL/CL intervals %s|%s'   % ( self.name , pp.name  ) )
        ## create ModelCofig 
        mc = ROOT.RooStats.ModelConfig ( 'MC' , 'ModelConfig for PL/CL intervals %s|%s' % ( self.name , pp.name  ) , ws )

        mc.SetPdf                   ( self.pdf  ) 
        mc.SetObservables           ( self.vars )
        mc.SetParametersOfInterest  ( self.poi ( dataset ) )
    ...
    # =========================================================================
    ## Get the confidence interval for the parameter using RooStats
    #  @code
    #  dataset   = ...
    #  pdf       = ,,,
    #  low, high = pdf.conf_interval ( 'sigma' ,  0.95 , dataset )    
    #  @endcode 
    #  @see RooStats::ProfileLikelihoodCalculator
    #  @see RooStats::ProfileLikelihoodInterval
    def conf_interval ( self , param , level , dataset ) :
        """ Get the confidence interval for the parameter using RooStats
        >>> dataset   = ...
        >>> pdf       = ,,,
        >>> low, high = pdf.conf_interval ( 'sigma' ,  0.95 , dataset )    
        - see `ROOT.RooStats.ProfileLikelihoodCalculator`
        - see `ROOT.RooStats.LikelihoodInterval`
        """
        assert 0 < level < 1 , 'Invalid confidence level %s' % level

        plc, mc, ws = self._plc_mc_ws ( param , dataset )
        pp          = self.parameter  ( param ) 
    ...
    # =========================================================================
    ## Get the upper limit for the parameter using RooStats
    #  @code
    #  dataset = ...
    #  pdf     = ,,,
    #  limit   = pdf.upper_limit ( 'sigma' ,  0.95 , dataset )    
    #  @endcode 
    #  @see RooStats::ProfileLikelihoodCalcualtor
    #  @see RooStats::ProfileLikelihoodInterval
    def upper_limit ( self , param , level , dataset ) :
        """ Get the upper limit for the parameter using RooStats
        >>> dataset = ...
        >>> pdf     = ,,,
        >>> limit   = pdf.upper_limit ( 'sigma' ,  0.95 , dataset )    
        ...
        plc.SetConfidenceLevel ( 1 - 2.0 * ( 1 - level ) ) ## ATTETNION!         
        plInt = plc.GetInterval()
    ...
    # =========================================================================
    ## Get the lower limit for the parameter using RooStats
    #  @code
    #  dataset = ...
    #  pdf     = ,,,
    #  limit   = pdf.lower_limit ( 'sigma' ,  0.95 , dataset )    
    #  @endcode 
    #  @see RooStats::ProfileLikelihoodCalculator
    #  @see RooStats::ProfileLikelihoodInterval
    def lower_limit ( self , param , level , dataset ) :
        """ Get the upper limit for the parameter using RooStats
        >>> dataset = ...
        >>> pdf     = ,,,
        >>> limit   = pdf.lower_limit ( 'sigma' ,  0.95 , dataset )    
