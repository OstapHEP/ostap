  ...
  const double        conflevel )
{
  ...
  ///
  typedef std::tuple<unsigned long,unsigned long,double> KEY    ;
  typedef std::pair<double,double>                       RESULT ; 
  typedef std::map<KEY,RESULT>                           MAP    ;
  typedef SyncedCache<MAP>                               CACHE  ;
  ///
  /// the key 
  const KEY key { accepted , rejected , conflevel } ;
  ///
  /// the cache
  ...
  //
  // The interval is:
  //   0 <= alpha <= e <= beta <= 1, such that  CDF ( beta ) - CDF ( alpha ) == conflevel 
  //
  ...
  //   0 <= alpha <= e <= beta <= 1,
  //   such that  CDF ( beta ) - CDF ( alpha ) == conflevel
  // due to this constrain for each (fixed) alpha beta is a function of alpha
  // and we need to find such alpha that minimizes the length of [alpha,beta] interval     
  //
  ...
  //
  const double min_e     = 1.e-4 * std::min ( e         , 1 - e         ) ;
  const double min_c     = 1.e-4 * std::min ( conflevel , 1 - conflevel ) ;
