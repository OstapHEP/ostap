...
{
  //
  const std::size_t M = cov.nRows () ;
  const std::size_t N = cov.nCols () ;
  if ( M != N ) { return MATRIX_IS_NOT_SQUARE ; }
  //
  Ostap::Math::GSL::Matrix splus { M , M } ;
  const Ostap::StatusCode sc = PINV ( cov , splus , tol ) ;
  if ( sc.isFailure()  ) { return sc ; }
  //
  if ( vif.size() != M ) { vif.resize ( M ) ; }
  for ( std::size_t i = 0 ; i < M ; ++i )
  {
    const double cii = cov.get ( i , i ) ;
    const double vi  = cii <= 0 ? std::numeric_limits<double>::infinity() : cii * splus.get ( i , i ) ;
    vif.set ( i , vi ) ;
  }
  return Ostap::StatusCode::SUCCESS ;                           
}
