...
( const Ostap::Trees::Branches&  right )
  : m_map () 
{
  for ( const auto& item : right.m_map )
    { m_map [ item.first ].reset ( item.second->clone() ) ; }
}
// ============================================================================
...
( const std::size_t index ) const
{
  ...
  BRANCHMAP::const_iterator cit = m_map.begin() ;
  std::advance ( cit , index ) ;
  return std::make_pair ( cit->first , cit->second.get()  ) ;
}
// =============================================================================
bool Ostap::Trees::Branches::add
( const std::string& name       ,
...
{
  // explicit loop 
  BRANCHMAP::const_iterator it = m_map.find ( key ) ;
  return ( m_map.end() != it ) ? it->second.get() : nullptr ; 
}
// ============================================================================
  ...
  for ( const auto& entry : lbranches ) 
    {
      ...
      const Ostap::IFuncTree* func = entry.second.get() ;
